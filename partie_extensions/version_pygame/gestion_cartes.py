...
from initialisation_course import numero_de_case
...
def configurations_joueurs(liste_joueurs):
    ...
    for j in liste_joueurs:
        config = input(f"Choisir un mode pour {j} ? ").upper()
        while config not in ["A", "M", "I"]: # redemander une saisie à l'utilisateur
            config = input(f"Recommencez! Choisir un mode pour {j} ? ").upper()
...
def choix_carte_manuel(nom_joueur, main_joueur):
    pygame.draw.rect(FEN, BLANC, (LARG - LARG/4 - LARG_CASE/2, LONG - LONG/10, LARG_CASE*8, LONG_CASE))
    FEN.blit(FONT2.render("Main " + nom_joueur + " : " + affiche_main(main_joueur), True, NOIR), (LARG - LARG/4, LONG - LONG/10))
    pygame.display.update()
    try:
        choix = int(input(f"{nom_joueur} quel déplacement souhaitez-vous effectuer ? "))
        while choix not in main_joueur: # redemander une saisie à l'utilisateur
            choix = int(input(f"Recommencez! {nom_joueur} quel déplacement souhaitez-vous effectuer ? "))
        return choix
    except:
...
def choix_deplacement(nom_joueur,dico_paquets,dico_defausses,configurations,dico_positions,affichage):
